<template>
  ...
  >
    <template #modal-header="{ close }">
      <b-container class="px-1" fluid>
        <close-row v-if="closable"
                   :title="getCapitalizeLang('deleteDictionaries')"
                   @close="reject()"
        ></close-row>
      </b-container>
    </template>
    ...
    <b-row>
      <b-col class="" sm="10">
        <b-form-group
            :label="getCapitalizeLang('group') + ':'"
            :label-for="properties.unique.id"
            content-cols-lg="7"
            content-cols-sm="7"
            label-class="py-0"
            label-cols-lg="3"
            label-cols-sm="3"
        >
          <div :id="properties.unique.id" class="text-truncate">
            <span v-if="unique">{{ getLang('unique') }}</span>
      ...
      <b-col class="" sm="10">
        ...
        >
          <b-form-input
              ...
              :placeholder="getCapitalizeLang('enterGroup')"
              :state="stateInput()"
              class="rounded-sm"
              size="sm"
              trim
  ...
  methods: {
    ...
    fetchData() {
      ...
      if (!this.isBlank(this.vocabularyStore.vocabulary)) {
        ...
        this.input.expected = this.unique ? this.getLang('unique') : this.getLang('notUnique')
        this.show = true
      }
    },
    onSelectSource() {
    ...
    confirm() {
      if (this.stateTrue()) {
        this.$store.dispatch(
            'deleteDictionariesByUnrepeatedAction',
            {
              vocabulary: this.vocabulary.vocabulary,
              unrepeated: this.unique,
              actual: this.input.actual,
